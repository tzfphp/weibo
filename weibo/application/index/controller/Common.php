...
class Common extends Controller
{
    ...
    public function _initialize()
    {
        ...
        if (is_null(Cookie::get('auto'))) {
            $this->isLogin();
        } else {

            $value = explode('|', $this->encryption(Cookie::get('auto'), 1));
            $ip = request()->ip(1);
   ...
   public static function encryption($value, $type = 0)
    {
        $key = md5(config("login.ENCTYPTION_KEY"));
    ...
    public function upload($file)
    {
        ...
        $src = '/uploads/' . strtolower($this->controller) . '/' . $info->getSaveName();
        if ($info) {
